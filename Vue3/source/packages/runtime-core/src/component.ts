...
export function setupComponent(
  ...
  // !vue 如果组件有状态，执行状态初始化，并返回setup的返回值
...
function setupStatefulComponent(
  ...
  if (setup) {
    // !vue 创建setup函数的上下文，并将其存储在组件实例的 setupContext 中
    const setupContext = (instance.setupContext =
      setup.length > 1 ? createSetupContext(instance) : null)

    // !vue 设置当前组件实例对象，配合 getCurrentInstance 可以在 setup 中获取当前组件实例
    const reset = setCurrentInstance(instance)
    // !vue 暂停跟踪
    pauseTracking()
    // !vue 调用 setup 函数, 为了避免意外的错误，使用错误处理函数包括了一层。
    const setupResult = callWithErrorHandling(
      ...
      [
        ...
        setupContext,
      ], // !vue 第四个参数就是 setup 函数执行时的参数，一个props和一个ctx
    )
    resetTracking()
    reset()
    ...
    if (isPromise(setupResult)) {
      ...
      // !vue 如果 setup 函数返回的不是一个Promise，则执行结果处理函数。
...
// !vue 如果setup函数返回值不是Promise时的结果处理逻辑
export function handleSetupResult(
  ...
  if (isFunction(setupResult)) {
    ...
    // !vue 如果返回值是个对象，则proxy代理，并赋值给 setupState, 将来取值先从这取
  ...
  // !vue 执行组件安装，执行的其他 Options 选项， 兼容vue2
  finishComponentSetup(instance, isSSR)
}
