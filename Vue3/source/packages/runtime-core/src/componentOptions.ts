...
export function applyOptions(instance: ComponentInternalInstance) {
  // !vue 获取实例的合并选项，这些选项包括组件的选项以及任何混入或扩展的选项
  const options = resolveMergedOptions(instance)
  // !vue 获取实例的代理对象，该对象表示组件实例
  const publicThis = instance.proxy! as any
  // !vue 获取实例上下文
  const ctx = instance.ctx

  // do not cache property access on public proxy during state initialization
  // !vue 在状态初始化期间禁用属性访问缓存
  shouldCacheAccess = false

  // call beforeCreate first before accessing other options since
  // the hook may mutate resolved options (#2791)
  // !vue 如果定义了 beforeCreate 钩子，则调用该钩子
      ...
      if (isFunction(methodHandler)) {
        ...
        if (__DEV__) {
          Object.defineProperty(ctx, key, {
            ...
            configurable: true, // !vue 可以被删除或修改其属性描述符
            enumerable: true, // !vue 在对象的枚举属性
            writable: true, // !vue 值可以被修改
          })
        } else {
